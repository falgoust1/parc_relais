...
var map = new maplibregl.Map({
  ...
  center: [-1.6658424068410043, 48.12729238983067 ], // lat/long
  zoom: 11.2,
...
map.on('load', async  () => {
    ...
    // TRACES LIGNES BUS STAR (via GEOJSON)  
...
data3 = 'https://data.rennesmetropole.fr/api/explore/v2.1/catalog/datasets/lignes-du-reseau-star-de-rennes-metropole/exports/geojson?lang=fr&timezone=Europe%2FBerlin';
  ...
  jQuery.when(
    ...
    { 'id': 'bhns',
      ...
      'paint' : { 'line-color': ['get', 'li_couleur_hex'], 'line-width' : 1.5}
    }, 'tram');
  });
...
map.addLayer({id: 'locations',
  ...
  //Bati
          ...
          'paint': {'fill-extrusion-color': '#e6f0ff',
...
function createPopUp(currentFeature) {
  ...
  const popup = new maplibregl.Popup({ closeOnClick: false })
    ...
    .setHTML(`<h3>${currentFeature.properties.nom}</h3><h4>Capacité totale :${currentFeature.properties.capaciteparking} places théoriques </h4>` + `<h4>Nombre de places disponibles :${currentFeature.properties.jrdinfosoliste}</h4>` + `<h4>Bornes electriques :${currentFeature.properties.jrdinfoelectrique}</h4>`)
    .addTo(map);
}
